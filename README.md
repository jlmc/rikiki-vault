...
| `migrate-format [--dry-run] [--yes]` | One-way: converts a vault from the older format (plaintext `manifest.json`, real filenames under `documents/`) to the current one, where paths and filenames are encrypted too. From a checkout, `scripts/migrate-format.sh <vault-dir> --dry-run\|--yes` is the ready-to-run form - see [FAQ 11](docs/faq/11-migrating-to-encrypted-paths.md). |
